    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">Data Tahun Ajaran - {{ $profil->nama_sekolah }}</h3>
        </div>
        <div class="card-body">
            ...
            @foreach ($tahunajaran as $p)
                <tr>
                  <td>{{ $loop->iteration }}</td>
                    ...
                    <td>
                        <a href="{{ route('tahunajaran.edit', $p->id) }}" class="btn btn-sm btn-warning" style="color: black">
                            <i class="fas fa-edit"></i> Edit
                        </a>
                        
                        @if ($p->status != 'Aktif') {{-- Hanya tampilkan tombol hapus jika status tidak aktif --}}
                            <form onsubmit="return confirm('Yakin mau hapus data?')" action="{{ route('tahunajaran.destroy', $p->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
