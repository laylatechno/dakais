...
@section('title','Halaman Tarik Tabungan')
@section('subtitle','Menu Tarik Tabungan')
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">Data Tarik Tabungan - {{ $profil->nama_sekolah }}</h3>
        </div>
          <!-- /.card-header -->
          <div class="card-body">
          ...
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tr>
              <th>No</th>
              <th>Tanggal Tarik Tabungan</th>
              <th>Nama Siswa</th>
              <th>Jumlah Tarik Tabungan</th>
              <th>PIC</th>
              <th>Aksi</th>
            </tr>
            </thead>
            <tbody>
              ...
              @foreach ($tarik_tabungan as $p)
                      <tr>
                  <td>{{ $loop->iteration }}</td>
                  <td>{{ $p->tanggal_tarik }}</td>
                  <td>{{ $p->siswa->nama_siswa }}</td>
                  <td>Rp. {{ number_format($p->jumlah_tarik) }}</td>
                  <td>{{ $p->pic }}</td>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Form Tarik Tabungan</h4>
              ...
              <div class="card card-primary">
                ...
                <form id="form-tabungan" action="" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="card-body">
                    ...
                    <div class="form-group" id="tanggal_tarik_container">
                      <label for="tanggal_tarik">Tanggal Tarik Tabungan</label>  
                      <input type="date" class="form-control " name="tanggal_tarik" id="tanggal_tarik" value="{{ now()->format('Y-m-d') }}">
                    </div>
                    ...
                    <div class="form-group" id="jumlah_tarik_container">
                      <label for="jumlah_tarik">Jumlah Tarik Tabungan</label>  
                      <input type="text" class="form-control jumlah_tarik" name="jumlah_tarik" id="jumlah_tarik" placeholder="Masukkan Jumlah Tarik Tabungan">
                    </div>
                    ...
                    <script>
                        $(document).ready(function () {
                            $('.jumlah_tarik').on('input', function (e) {
                    ...
                    <div class="form-group" id="pic_container">
                      <label for="pic">PIC Tarik Tabungan</label>  
                ...
                <div class="card card-primary">
                  ...
                  <form id="form-tabungan-edit" action="" method="POST">
                    ...
                    <div class="card-body">
                      ...
                      <div class="form-group" id="tanggal_tarik_edit_container">
                        <label for="tanggal_tarik_edit">Tanggal Tarik Tabungan</label>  
                        <input type="date" class="form-control " name="tanggal_tarik" id="tanggal_tarik_edit">
                      </div>
                      <div class="form-group">
                      ...
                      <div class="form-group" id="jumlah_tarik_edit_container">
                        <label for="jumlah_tarik_edit">Jumlah Tarik Tabungan</label>  
                        <input type="text" class="form-control jumlah_tarik_edit" name="jumlah_tarik" id="jumlah_tarik_edit" placeholder="Masukkan Jumlah Tarik Tabungan" readonly>
                      </div>
                      ...
                      <script>
                          $(document).ready(function () {
                              $('.jumlah_tarik_edit').on('input', function (e) {
                                  var inputVal = $(this).val().replace(/[^\d]/g, ''); // Hapus semua karakter non-digit
                                  var formattedVal = addThousandSeparator(inputVal);
                                  $(this).val(formattedVal);
                              });
                      ...
                      <div class="form-group" id="pic_edit_container">
                        <label for="pic_edit">PIC Tarik Tabungan</label>  
...
<script>
  $(document).ready(function() {
      $('#form-tabungan').submit(function(event) {
          ...
          $.ajax({
              url: '{{ route("tarik_tabungan.store") }}',
              type: 'POST',
              data: formData,
              ...
              error: function(xhr) {
                  ...
                  // Cek jika terdapat pesan kesalahan khusus untuk tabungan tidak mencukupi
                  if (xhr.responseJSON.error && xhr.responseJSON.error === 'Tabungan tidak mencukupi untuk ditarik') {
                      Swal.fire({
                          title: 'Error!',
                          text: 'Tabungan tidak mencukupi untuk ditarik',
                          icon: 'error',
                          confirmButtonText: 'OK'
                      });
                  } else {
              ...
              tabungan: 'GET',
                ...
                success: function(data) {
                  ...
                  function addThousandSeparator(num) {
                    ...
                    var formattedSpp = addThousandSeparator(data.jumlah_tarik); // Format jumlah_tarik
                    $('#jumlah_tarik_edit').val(formattedSpp);

                    // Mengisi data pada form modal
                    $('#id').val(data.id); // Menambahkan nilai id ke input tersembunyi
                    $('#tanggal_tarik_edit').val(data.tanggal_tarik);
